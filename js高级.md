...
- JS引擎在执行代码之前，会在堆内存中创建全局对象（window），该对象可以访问所有作用域
- JS引擎内部有一个执行上下文栈，是用于执行代码的调用栈
  - 执行上下文栈中可以创建多个执行上下文，去执行不同的代码（一个函数调用会创建一个执行上下文）
  - 代码进入到执行上下文中，执行
  - 在代码进入到执行上下文中之前，需要先经过JS引擎（V8）编译成可执行代码，AST树、字节码、机器码...
  - 在JS引擎编译之前，会对作用域进行提升，在内存中，将变量、函数的声明添加到VO对象，一个VO对应一个执行上下文，相互关联；全局上下文对应的VO就是GO（window=this）
    - 全局代码执行前
    - VO用于存放变量、函数...
    - 变量提升，只把变量的声明加入了VO；
    - 函数把声明加入了VO，同时又创建了一个存放函数的对象，声明引入了这个对象的地址；
      - 函数会被提前声明，放到最前面（JS本身的优化）
  ...
  - 当函数开始执行之前，会再创建一个新的执行上下文，放在全局执行上下文的上面，并关联一个VO对象，用于存放这个函数中的变量、函数
    - 函数的执行上下文，是函数执行上下文，对应的VO是AO对象
    - AO会使用arguments作为初始化，初始化值就是传入的参数
  - 函数之前ing
    - AO中的变量进行赋值（值是源代码中变量对应的值）
    - 执行具体代码
  - 函数执行上下文完成后
    - 会跳出执行上下文栈（销毁、被回收），继续执行下一个执行上下文，这里下一个就是最底层的全局代码，全局代码再执行全局的下一行代码，是函数再重复函数上下文执行过程
  - 函数相互调用
    - 父函数创建执行上下文、VO对象，进行变量赋值，执行代码
    - 当执行到子函数的时候，会再次创建一个子函数的执行上下文、VO对象，进行变量赋值，执行代码
    - 当子函数代码执行完成后，销毁、回收子函数的执行上下文，跳出执行上下文栈，父函数执行上下文变成栈顶
    - 再继续执行父函数的代码，直到完成，跳出栈
  ...
  - 如果是执行函数代码，在函数定义的时候，函数就确定了作用域链的内容
    ...
    - 那函数在定义的时候，是怎么确定的作用域链？内容是作用域（VO对象）
      ...
      - 查找顺序是
        ...
        - 自身作用域（VO对象）、A作用域（VO对象）、B作用域（VO对象）、全局作用域（GO对象）
...
- 左侧为栈，底层是window全局
- 右侧是堆内存
- 全局代码执行前，创建全局执行上下文，创建并关联GO对象，存放全局代码中的msg变量、foo函数（放的是foo函数的内存地址）
- foo函数要在内存新建，才会有一个存放地址，在foo函数对象中有一个作用域链，作用域链是一个对象列表，列表内容在这里是全局执行上下文的VO，即GO
...
- foo函数执行了，nema被赋值为foo函数中name的值；foo执行会返回bar函数，这个bar函数被放到了（全局）全局执行上下文的GO对象，引入地址就是bar函数的内存存放地址
...
- 以上代码执行过程(foo函数中有声明msg)
  ...
  - 函数执行：执行函数中的代码，第一行中的msg会去函数的VO对象中查找，因为var 变量名提示，所以函数的VO对象中有msg但初始值为undefined，要执行到第二行才会赋值
...
![闭包的内存泄漏测试](C:\Users\admin\Desktop\系统笔记\img_js_高级\闭包的内存泄漏测试.png)
